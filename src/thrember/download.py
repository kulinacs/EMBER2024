...
def download_dataset(download_dir, split="all", file_type="all"):
    ...
    if file_type not in VALID_FILES:
        raise ValueError("file_type must be in {}".format(
            ", ".join(VALID_FILES)))
    ...
    for split in splits:
        ...
        for file_type in file_types:
            ...
            zip_path = hf_hub_download(
                repo_id="joyce8/EMBER2024", filename=file_name, repo_type="dataset", local_dir=download_dir)
            print("Unzipping...")
    ...
    if "challenge" in splits:
        zip_path = hf_hub_download(
            repo_id="joyce8/EMBER2024", filename="challenge.zip", repo_type="dataset", local_dir=download_dir)
        with zipfile.ZipFile(zip_path, "r") as f:
            f.extractall(".")
        os.remove(zip_path)
    return
...
def download_models(download_dir):
    ...
    model_files = [file_name for file_name in list_repo_files(
        repo_id) if file_name.endswith(".model")]
